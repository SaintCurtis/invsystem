...
export default function HomeNavbar() {
  ...
  return (
    ...
    <p className='text-slate-400 font-semibold'>Hello, WELCOME</p>
    <span className='text-sm'>Samaco</span>
    </div>
    </div>
    <nav className='sticky mt-6 flex space-x-4'>
    {navLinks.map((item,i)=> {
      return (
        <Link key={i} href={item.href} className={`${pathname===item.href?'border-b-2 border-blue-600':'py-1'}`}>
        {item.title}
        </Link>
      )
    })}
